...
public class DataSource {
    ...
    public static final String CONNECTION_STRING = "jdbc:sqlite:/Users/anna/Java Programs/Database 2/" + DB_NAME;
    ...
    public List<SongArtist> queryArtistFromSong(String querySong, SortOrder sortOrder) {

        StringBuilder sb = new StringBuilder(QUERY_ARTIST_ALBUM_AND_TRACK_BY_SONG_TITLE_START);
        sb.append(querySong).append("\"");
        if (sortOrder != SortOrder.ORDER_BY_NONE) {
            sb.append(" ORDER BY ");
        ...
        try (Statement statement = connection.createStatement();
            ...
            while (resultSet.next()) {
                ...
                songArtist.setTrack(resultSet.getInt(4));
                songArtist.setSongTitle(resultSet.getString(1));
            ...
            System.out.println("Something went wrong in queryArtistBySong");
            e.printStackTrace();
            return null;
        }
    }
    ...
    public List<SongArtist> querySongsInfo(String songTitle) {
        try {
            ...
            while (resultSet.next()) {
                SongArtist songArtist = new SongArtist();
                songArtist.setSongTitle(songTitle);
                songArtist.setTrack(resultSet.getInt(COLUMN_SONG_TRACK));
                songArtist.setArtistName(resultSet.getString(COLUMN_ARTIST_NAME));
                songArtist.setAlbumName(resultSet.getString("album"));
                songArtists.add(songArtist);
            }
